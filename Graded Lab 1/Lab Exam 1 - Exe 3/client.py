...
def attack(tn: Telnet):
    ...
    #Basically, we use the padding oracle for decryption of ciphertext blocks. Since the methods are just copied from M4, we just make minor changes, which are described in the appropriate methods.
    ...
    #In each iteration, we use the padding oracle to decrypt the last byte of the first non-iv ciphertext to get the last salt-byte, since the salt is in the first non-iv plaintext. Then we send the last-salt byte als ascii character to the server.
    ...
    for _ in range(1000):
        ...
        decrypted_message = decrypted_message + decrypted_block #add newly found partial plaintext to the plaintexts we have found so far. note that only the last byte is decrypted.
