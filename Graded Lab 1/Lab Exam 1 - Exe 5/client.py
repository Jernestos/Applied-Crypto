...
def attack(tn: Telnet):
    ...
    First, we setup our padding oracle, and then get some encrypted message, encrypted under the server key. We then decrypt this message (in code called solution) and send it back to the server. For more information, see below and comments in methods used. It's described there.
    ...
    #the idea is the following: The server sends back some encrypted plaintext, encrypted under the server key
    #we then reflect it back to the server. The server parses it (read_message), and in the method "read_command(...)", it can correcly decrypt it with no padding error and no decoding error, because it has the key to decrypt (and it was the one who padded, encrypted and sent it to me, the client). So the server decrypts it back to the original (error) message (error message because get_some_encrypted_server_message() likely sent some ciphertext that is either not utf8 decodable or has an invalid padding), that is certainly not "hello". So the next instruction, because up until this point, no exception has been thrown, is to execute "exec_command(...)". Observe that in "exec_command(...)", because the decrypted ciphertext is not equals to "hello", we get to the else branch, raising an ValueError with an error message that contains the flag. This error message, including the flag embedded into it, gets encrypted. The resulting ciphertext (in code as variable "flag") is sent to me, the client. Using the padding oracle, I can decrypt it.
    
    flag = send_my_enc_stuff(tn, ciphertext) #we receive an encrypted error message with the flag in it -> we decrypt this message to get the flag.
    print("Encryped message with flag in it: ",end="")
    print(flag)
 
    (iv, individual_ctxts, num_blocks) = get_individual_blocks_from_ciphertext(flag) #structure cipheretxt; num_blocks is number of blocks, not counting the IV block; everything is in bytes
        
    decrypted_message = b''
        
        #we can safely assume that we are dealing here with blocks such that there is at least 2 a "truly" ciphertext, in addition to the IV. This is because the server sends back encrypted messages with at least 2 non-iv cipher text blocks
    number_of_padding_bytes = find_padding_length(ordered_lengths[1],individual_ctxts[-2],individual_ctxts[-1]) #determine the number of padding bytes, for later, when we unpad the plaintext
        
    previous_ciphertext_block = iv
    for current_ciphertext_block in individual_ctxts:
        print("Starting decryption for next block")
        decrypted_block = decrypting_block(ordered_lengths[1], previous_ciphertext_block, current_ciphertext_block) #get plaintext p_i
        decrypted_message = decrypted_message + decrypted_block #add p_i to what we know already about the plaintext
    ...
    print(decrypted_message[:-number_of_padding_bytes].decode()) #here, we can read the flag
