...
def attack(tn: Telnet):
    ...
    previous_block = iv
    for block_number_i in range(num_blocks - 1): #iterate through blocks
...
#        print(decrypted_block)
    ...
    plaintext_in_hex = bytes.hex(decrypted_stuff) 
    print("plaintext in hex: ", end="")
    print(plaintext_in_hex)
    
    plaintext_in_ascii = bytes.fromhex(plaintext_in_hex).decode()
    print("plaintext_in_ascii: ", end="")
    print(plaintext_in_ascii)
