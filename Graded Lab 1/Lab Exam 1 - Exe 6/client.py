...
#Reuse of code for task m2, m3, m4.
...
#    print("(discounting IV) num blocks: ", end="")
#    print(num_blocks)
...
#    print("-" * 20)
    ...
    current_ciphertext_block = individual_ctxts[0] #we only need 1 non-iv ciphertext block, because we are going to put the flag into the first plaintext block
    print("current_ciphertext_block: ", end="")
    print(current_ciphertext_block)
    print("Starting decryption for next block")
    decrypted_block = decrypting_block(ordered_lengths[1], current_ciphertext_block) #block D_1(C1)
    print("Finished decryption of current block")
    
    #we need the padded plaintext block
    
    #idea: from construction, we have a ciphertext of the form IV | C1 (the other non-IV blocks are disregarded) since we put the flag command into the first plaintext block
    #note that "flag" is only 4 bytes, so it fits into a 16 byte block, which then needs to be padded
    #we can decrypt C1 to (unpadded) plaintext block p1. 
    #Let decryption of C1 (before it gets xored with previous ciphertext block) be D_1(C1). We can compute D_1(C1) (as above  the decrypted_block) with the padding oracle.
    #let plaintext block (padded) with the flag command be P_with_flag.
    # Now we need to choose a new iv block such that NEW_IV XOR D_1(C1) = P_with_flag.
    #we can solve for NEW_IV = D_1(C1) XOR P_with_flag.

    d_1 = decrypted_block
    print("decrypted_block: ", end="")
    print(decrypted_block)
    p_flag = pad("flag".encode(), AES.block_size) #P_with_flag, pad this 16 byte block, containing "flag"
    print(str(len(p_flag)) + " p_flag: ", end="")
    print(p_flag)
    new_iv = strxor(d_1, p_flag)
    print("new_iv: ", end="")
    print(new_iv)
    
    new_ciphertext = new_iv + individual_ctxts[0] #new_iv + original first non-iv ciphertext, we are only interested in the first non-iv cypertext, because our plaintext consists of only 1 block.
    print("new_ciphertext: ", end="")
    print(new_ciphertext)
    
    new_ciphertext_in_hex = bytes.hex(new_ciphertext) #in hex.
    print("new_ciphertext_in_hex: ", end="")
    print(new_ciphertext_in_hex)
    
    #on the serverside, new_ciphertext_in_hex will decrypted to "flag".
    res = send_encrypted_flag_command(tn, new_ciphertext_in_hex) #we get an encrypted response, with the flag in it
    print("res: ", end="")
    print(res)
    
    
    #debug to check if it really decrypts to the unpadded plaintext "flag"
    print("-" * 20)
    ...
    #we can safely assume that we are dealing here with blocks such that there is at least 2 a "truly" ciphertext, in addition to the IV, because by inspection of the server code, the reply sent back is at least 2 blocks long (not counting iv block)
